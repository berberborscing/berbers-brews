...
public class GoldPerceptionEffect extends StatusEffect {
    public GoldPerceptionEffect(StatusEffectCategory statusEffectCategory, int color) {
            ...
            for (int x = -range; x <= range; x++) {
                for (int y = -range; y <= range; y++) {
                    for (int z = -range; z <= range; z++) {
                        ...
                        {
                            world.playSound(entity, targetPos, SoundEvents.BLOCK_NOTE_BLOCK_BELL.value(), SoundCategory.BLOCKS, 1f, pitch);
                        }
                    }
                }
            }
